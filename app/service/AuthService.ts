...
import type { AuthResponse, LoginData } from "../Types/authTypes";
...
class AuthService {
  ...
  async login(credentials: LoginData): Promise<AuthResponse> {
    try {
      ...
      if (response.data?.token) {
        ...
        // Dispatch an event after successful login
        this.eventTarget.dispatchEvent(new Event("authChange"));
      }
  ...
  unsubscribeFromAuthChanges(callback: () => void) {
    this.eventTarget.removeEventListener("authChange", callback);
  }
}
